  ...
  if (!user) {
    return (
      <Card style={styles.formCard}>
        <Title level={4}>
          <span style={styles.formTitle}> Bienvenue sur InstaSlut !</span>
          <br />
          Veuillez vous <Link to="/authform">connectez</Link> ou vous{" "}
          <Link to="/authform">enregistrer</Link> pour montrer au monde entier à quel point vous êtes une grosse merde !
        </Title>
      </Card>
    );
  }
    ...
    <Card
      ...
      title={
        <Title level={4} style={styles.formTitle}>
          {selectedId ? "Editing" : "Share"} Story
        </Title>
      }
    >
      <Form
